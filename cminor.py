...
logger = logging.getLogger("cminor")
...
class cMinorInfo(coap.Resource):
    """Global info resource about cMinor v2"""
    # This is the coap resource that will provide informations about
    # cMinor to the clients making GET request
...
class SparqlQuery(coap.Resource):
    ...
    async def render_get(self, request):
        ...
        if request.payload == b'':
            ...
            res, code = rdf_endpoint.query(request.payload.decode())
            if code:
            ...
            if len(coap_request_options) > 1:
                ...
                for option in coap_request_options:
                    ...
                    if len(parse_option) > 1 and "format" == parse_option[0]:
                        ...
                        res, code = rdf_endpoint.update(request.payload.decode(), format=parse_option[1])
                        logger.debug("Result: {}; code: {}".format(res, code))
                        break
            else:
                res, code = rdf_endpoint.update(request.payload.decode())
                logger.debug("Result: {}; code: {}".format(res, code))
            if code:
...
def cminor(a4="default", a6="default", port=5683,  # custom ip:port address for cMinor
    ...
    """Invokes cMinor in a programmatic way (no CLI)

    Parameters
    ----------
    a4 : str, optional
        defaults to 'default' (i.e. 127.0.0.1), should be the ipv4 ip address
        where cMinor is listening.

    a6 : str, optional
        defaults to 'default' (i.e. ::ffff:127.0.0.1), should be the ipv6 ip
        address where cMinor is listening.

    port : int, optional
        default to CoAP default port 5683, it's the port on which cMinor is
        listening
    ...
    logging.basicConfig(level=logging.INFO, format=logFormat, filename="./cminor_log.log")
...
def main(addressV4, addressV6, port, endpoint, loop=asyncio.get_event_loop()):
    ...
    root.add_resource(('info',), cMinorInfo())
    ...
    if addressV4 == "default":
        ...
        cMinorAddress = "coap://{}:{}".format(addressV4,port)
    print("\n/-------------------------------------------------------\\")
    print("| Information: \t{}/info".format(cMinorAddress))
    print("| Query: \t{}/sparql/query".format(cMinorAddress))
    print("| Update: \t{}/sparql/update".format(cMinorAddress))
    print("| Subscribe: \t{}/sparql/subscription".format(cMinorAddress))
    print("\\-------------------------------------------------------/")
    ...
    logger.info("cMinor was stopped")


if __name__ == '__main__':
    # command line parsing
    parser = argparse.ArgumentParser(description="cMinor server")
    parser.add_argument(
        "--address", metavar=("CMINOR_ADDRESS"), 
        default="default", type=str, help="cMinor server ip address")
    parser.add_argument(
        "--port", metavar=("CMINOR_PORT"), 
        default=5683, type=int, help="cMinor server port")
    parser.add_argument(
...
""")
    ...
    logger.info("cMinor configuration complete!")
